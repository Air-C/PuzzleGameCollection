...
namespace PGCRefactor.InfrastructureLayer.AssetsLoadModule
{
    public class AssetsLoadModule : IAssetsProvider
    {
        ...
        public Dictionary<PrefabEnum, GameObject> prefabDictionary = new ();
        ...
        public IEnumerator LoadAllAssets()
        {
            ...
            {
                prefabDictionary[PrefabEnum.DestroyParticle] = particle;
            });
            ...
            {
                prefabDictionary[PrefabEnum.AudioSourcePrefab] = prefab;
            });
        ...
        public bool GetPrefab(PrefabEnum type, out GameObject prefab)
        {
            bool exist = prefabDictionary.TryGetValue(type, out prefab);
            if (!exist)
            {
        ...
        {
            if (!prefabDictionary.TryGetValue(prefabEnum, out var prefab))
            {
                throw new KeyNotFoundException(prefabEnum.ToString()+" : Prefab doesn't exist");
            }
            return prefab;
        }
    }
